...
!
!       ! Tracers added when using Diel Vertical Migration (dvm)
!
...
module generic_COBALT
  ...
  use cobalt_eco, only : dvm_conservation_totn, dvm_conservation_totc, dvm_conservation_totp
  use cobalt_eco, only : dvm_conservation_totfe, dvm_conservation_totsi
  use cobalt_eco, only : dvm_apply_tendencies, dvm_add_layer_integrals
  ...
  namelist /generic_COBALT_nml/ co2_calc, do_14c, do_nh3_atm_ocean_exchange, scheme_nitrif, debug, &
     imbalance_tolerance,as_param_cobalt, do_dvm
...
# include "version_variable.h"
  ...
  subroutine user_add_params(param_file)
    ...
    !
    ! Diel Vertical Migration (DVM) parameters are read by this subroutine from COBALT_eco 
    ! DVM parameters are read unconditionally: because NUM_ZOO is fixed at 5, the
    ! food-web loops iterate over zoo(4:5) even when do_dvm is false, so those groups
    ! must hold finite (default) parameter values. The DVM *physics* remains gated by
    ! do_dvm. (If NUM_ZOO becomes a runtime value this can revert to a do_dvm guard.)
    call dvm_add_params(param_file, zoo, cobalt)
...
!            implementation='duce',   &
    ...
    if (do_dvm) then
       call cobalt_eco_add_tracers(tracer_list, package_name)
    endif
...
!
    ...
    do m = 1,NUM_ZOO !{
       ...
       ! domain (outside the i,j,k loop); each cell below only overwrites the
       ! entries a given predator group actually uses.  The migrating-group rows
       ! (m=4,5) therefore stay 0 when DVM is off, keeping total_filter_feeding and
       ! the jzloss/jhploss sums bit-for-bit identical to stock COBALT.  Do NOT
       ! move or drop this zeroing.
       do n = 1,NUM_PREY !{
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec; !{
       ...
       ! Index map (DVM branch): 1-4 phyto, 5 bact, 6-8 zoo(1:3), 9-10 migrating
       ! zoo(4:5), 11 detritus. Adding the migrating groups pushed detritus to the end
       ! (it must stay last per the [phyto,bact,zoo,det] convention that the NUM_PHYTO+1+n
       ! index formulas rely on).  prey_vec(9:10) are the migrating groups and are 0
       ! unless do_dvm (see zoo f_n init in user_allocate_arrays), so the m=3/4/5 and
       ! higher-predator terms that reference them vanish (+0) when DVM is off.
       ...
       !
       ! prey_vec(9)=zoo(4) and prey_vec(10)=zoo(5) are the migrating groups, which
       ! are 0 unless do_dvm (see zoo f_n init in user_allocate_arrays).  This block
       ! is therefore left deliberately unguarded: the migrating terms vanish (+0)
       ! and it is bit-for-bit stock COBALT when DVM is off.  See DVM_REFACTOR_FOLLOWUPS.md.
    ...
    if (do_dvm) then
        call dvm_migration(zoo, phyto, cobalt, rho_dzt, ilb, jlb, isc, iec, jsc, jec, nk)
    endif
...
!
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec  !{
          ...
          if (do_dvm) pre_totn(i,j,k) = pre_totn(i,j,k) + dvm_conservation_totn(cobalt, i, j, k, tau, grid_tmask(i,j,k))
         ...
         ! >>
          pre_totc(i,j,k) = (cobalt%p_dic(i,j,k,tau) + &
                     ...
                     cobalt%p_nsmz(i,j,k,tau)        + cobalt%p_nmdz(i,j,k,tau) + &
                     cobalt%p_nlgz(i,j,k,tau)))*grid_tmask(i,j,k)
          if (do_dvm) pre_totc(i,j,k) = pre_totc(i,j,k) + dvm_conservation_totc(cobalt, i, j, k, tau, grid_tmask(i,j,k))
          ...
          if (do_dvm) pre_totp(i,j,k) = pre_totp(i,j,k) + dvm_conservation_totp(cobalt, zoo, i, j, k, tau, grid_tmask(i,j,k))
         net_srcp(i,j,k) = cobalt%jpo4_iceberg(i,j,k)*dt*grid_tmask(i,j,k)
          ...
          if (do_dvm) pre_totfe(i,j,k) = pre_totfe(i,j,k) + dvm_conservation_totfe(cobalt, i, j, k, tau, grid_tmask(i,j,k))
          ...
          if (do_dvm) pre_totsi(i,j,k) = pre_totsi(i,j,k) + dvm_conservation_totsi(cobalt, i, j, k, tau, grid_tmask(i,j,k))
    enddo; enddo ; enddo  !} i,j,k
...
!
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec  !{
     ...
     if (do_dvm) call dvm_apply_tendencies(cobalt, zoo, dt, grid_tmask(i,j,k), i, j, k, tau)
     enddo; enddo ; enddo  !} i,j,k
!
...
!
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec  !{
      if (dzt(i,j,k).gt.cobalt%min_thickness) then
          post_totn(i,j,k) = (cobalt%p_no3(i,j,k,tau) + cobalt%p_nh4(i,j,k,tau) + &
                     ...
                     cobalt%p_nsmz(i,j,k,tau) + cobalt%p_nmdz(i,j,k,tau) + &
                     cobalt%p_nlgz(i,j,k,tau))*grid_tmask(i,j,k)
          if (do_dvm) post_totn(i,j,k) = post_totn(i,j,k) + dvm_conservation_totn(cobalt, i, j, k, tau, grid_tmask(i,j,k))
         ...
         if (abs(imbal).gt.imbalance_tolerance) then
          ...
          post_totc(i,j,k) = (cobalt%p_dic(i,j,k,tau) + &
                     ...
                     cobalt%p_nsmz(i,j,k,tau) + cobalt%p_nmdz(i,j,k,tau) + &
                     cobalt%p_nlgz(i,j,k,tau)))*grid_tmask(i,j,k)
          if (do_dvm) post_totc(i,j,k) = post_totc(i,j,k) + dvm_conservation_totc(cobalt, i, j, k, tau, grid_tmask(i,j,k))
        imbal = (post_totc(i,j,k) - pre_totc(i,j,k) - net_srcc(i,j,k))*86400.0/dt*1.03e6
         if (abs(imbal).gt.imbalance_tolerance) then
          ...
          post_totp(i,j,k) = (cobalt%p_po4(i,j,k,tau) + cobalt%p_pdi(i,j,k,tau) + &
                     ...
                     cobalt%p_nlgz(i,j,k,tau)*zoo(3)%q_p_2_n  + &
                     bact(1)%q_p_2_n*cobalt%p_nbact(i,j,k,tau))*grid_tmask(i,j,k)
          if (do_dvm) post_totp(i,j,k) = post_totp(i,j,k) + dvm_conservation_totp(cobalt, zoo, i, j, k, tau, grid_tmask(i,j,k))
         ...
         if (abs(imbal).gt.imbalance_tolerance) then
          ...
          post_totfe(i,j,k) = (cobalt%p_fed(i,j,k,tau) + cobalt%p_fedi(i,j,k,tau) + &
                     cobalt%p_felg(i,j,k,tau) + cobalt%p_femd(i,j,k,tau) + cobalt%p_fesm(i,j,k,tau) + &
                     cobalt%p_fedet(i,j,k,tau))*grid_tmask(i,j,k)
          if (do_dvm) post_totfe(i,j,k) = post_totfe(i,j,k) + dvm_conservation_totfe(cobalt, i, j, k, tau, grid_tmask(i,j,k))
         ...
         if (abs(imbal).gt.imbalance_tolerance) then
          ...
          post_totsi(i,j,k) = (cobalt%p_sio4(i,j,k,tau) + cobalt%p_silg(i,j,k,tau) + &
                     cobalt%p_simd(i,j,k,tau) + cobalt%p_sidet(i,j,k,tau))*grid_tmask(i,j,k)
          if (do_dvm) post_totsi(i,j,k) = post_totsi(i,j,k) + dvm_conservation_totsi(cobalt, i, j, k, tau, grid_tmask(i,j,k))
    ...
    if (do_dvm) call dvm_add_layer_integrals(cobalt, zoo, rho_dzt, ilb, jlb, tau)
...
!
  ...
  subroutine user_allocate_arrays
    ...
    !
    ! do_dvm=.false. INVARIANT: f_n is allocated and zero-initialized for ALL
    ! NUM_ZOO(=5) groups here, but the migrating groups zoo(4:5) are only filled
    ! from tracer fields inside the "if (do_dvm)" block in update_from_source.
    ! When DVM is off they stay 0.0, so every unguarded DVM ingestion/predation
    ! term (prey_vec(9:10), the m=4,5 blocks, the higher-predator food2) evaluates
    ! to +0 and reproduces stock COBALT bit-for-bit.  Do NOT drop this init.
    do n = 1, NUM_ZOO
